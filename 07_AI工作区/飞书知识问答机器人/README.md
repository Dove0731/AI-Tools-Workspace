...
- 无可靠参考答案：明确标注“待确认/未命中可靠来源”，不自动采用本人回复，不自动写入场景数据库。
- 正式业务上线：未完成。
...
- 回复 `驳回 QA-编号`：终止发送并留痕。
- 当知识库无可靠资料时，回答保留“待确认/未命中可靠来源”；不再识别 `补充 QA-编号：标准答案`，也不会把回复自动发送到原会话或用于学习。
...
- `runtime/qa-knowledge.sqlite`：历史遗留场景库，当前逻辑不再读取或写入；文件暂时保留，未执行删除。
